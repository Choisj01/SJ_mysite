...
/*************************************************************** 
    ...
    -> 요소 뒤 대괄호는 속성선택자 (css 원래문법)
    [id!=email2] != 는 같지 않다(제이쿼리용 문법)    
***************************************************************/
$(` form.logF input[type=text][id!=email2],
form.logF input[type=password]`)
    // .on('blur',function(){})
...
/*****************************************************
   ...
   1. keypress : 키가 눌려졌을때
   2. keyup : 키가 눌렸다가 올라올떄
   3. keydown : 키가 눌려져서 내려가 있을떄
   -> 과연 글자가 입력되는 순간은 어떤 키보드이벤트를 
   사용해야할까? ->>
...
.on('keyup',function(){
    ...
    // 현재 입력아이디가 'email1'이면 석택박스 값
    // 아니면 두번째 이메일창에 입력하는 것이므로
    // 두번쨰 이메일 입력값을 뒷주소로 설정함! 
...
$('#btnj').click(e=>{
    ...
    if(pass){
        ...
        /* 
            ...
            - 비동기 통신이란? 쉽게 말해서 페이지가
            새로 고쳐지지 않고 그대로 있으면서 일부분만
            서버통신을 하는 것을 말한다!
            - 제이쿼리는 POST 방식으로 ajax를 처리하는 
            메서드를 제공한다!

            [ POST방식 Ajax 메서드 ]
            $.post(URL,data,callback)
            $.post(전송할 페이지,전송할 데이터,전송후콜백함수)
        ...
        */
        ...
        // 민감한 입력 데이터페이지가 다시 돌아와서 
        // 보이면 안되기때문에 히스토리를 지우는 
        // replace()로 이동한다! 
        // location.replace('login.php');
    } ///////// if : 통과시 ////////
    else{
